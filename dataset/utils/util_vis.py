...
from dataset.utils.poses import cam2world
...
def color_border(images, colors, width=3):
    ...
    for i, image in enumerate(images):
        image_pad = torch.ones(3, image.shape[1] + width * 2, image.shape[2] + width * 2) * (
...
def plot_save_poses(cam_depth, fig, pose, pose_ref=None, path=None, ep=None):
...
def plot_save_poses_whu(cam_depth, fig, pose, pose_ref=None, path=None, ep=None):
    _, _, cam = get_camera_mesh(pose, depth=cam_depth)
    cam = cam.numpy()
    ...
    # set up plot window(s)
    ax1 = fig.add_subplot(111, projection="3d")  # Use a single subplot for top-down view
    setup_3D_plot(ax1, elev=40, azim=50, lim=edict(x=(-0.6, 0.6), y=(-0.6, 0.6), z=(-0.6, 0.6)))
    # plt.subplots_adjust(left=0, right=1, bottom=0, top=0.95, wspace=0, hspace=0)
    # plt.margins(tight=True, x=0, y=0)
    ...
    if pose_ref is not None:
        for i in range(3, N, 4):
            ax1.plot(cam_ref[i, :, 0], cam_ref[i, :, 1], cam_ref[i, :, 2], color='gray', linewidth=1)
            ax1.scatter(cam_ref[i, 5, 0], cam_ref[i, 5, 1], cam_ref[i, 5, 2], color='gray', s=40)

    for i in range(3, N, 4):
        # ax1.plot(cam[i, :, 0], cam[i, :, 1], cam[i, :, 2], color='#358FF0')
        # ax1.scatter(cam[i, 5, 0], cam[i, 5, 1], cam[i, 5, 2], color='#358FF0', s=40)
        if i %4 == 1:
            ax1.plot(cam[i, :, 0], cam[i, :, 1], cam[i, :, 2], color='pink')
            ax1.scatter(cam[i, 5, 0], cam[i, 5, 1], cam[i, 5, 2], color='pink', s=40)
        else:
            ax1.plot(cam[i, :, 0], cam[i, :, 1], cam[i, :, 2], color='cornflowerblue')
            ax1.scatter(cam[i, 5, 0], cam[i, 5, 1], cam[i, 5, 2], color='cornflowerblue', s=40)

        if pose_ref is not None:
            # Connect corresponding points with a red line
            ax1.plot([cam[i, 5, 0], cam_ref[i, 5, 0]],
                     [cam[i, 5, 1], cam_ref[i, 5, 1]],
                     [cam[i, 5, 2], cam_ref[i, 5, 2]], color='red', linewidth=3.5)
