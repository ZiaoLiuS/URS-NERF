...
class Lie():
    ...
    def se3_to_SE3_large(self, wu):  # [...,3]
        w, u = wu.split([3, 3], dim=-1)
        wx = self.skew_symmetric(w)
        theta = w.norm(dim=-1)[..., None, None]
        I = torch.eye(3, device=w.device, dtype=torch.float32)
        ...
        V = I + B * wx + C * wx @ wx
        t =  V @ u[..., None]
        dis = self.average_distance(t)
        Rt = torch.cat([R, (V @ u[..., None])*0.5/dis], dim=-1)
        return Rt

    def average_distance(self,points):
        # 计算欧几里得距离
        distances = torch.norm(points[:, :, None] - points[:, None, :], dim=1)
    ...
    def se3_to_SE3_scale(self, wu):  # [...,3]
        w, u, s = wu.split([3, 3 ,1], dim=-1)
        wx = self.skew_symmetric(w)
        theta = w.norm(dim=-1)[..., None, None]
        I = torch.eye(3, device=w.device, dtype=torch.float32)
        A = self.taylor_A(theta)
        ...
        R = I + A * wx + B * wx @ wx
        V = I + B * wx + C * wx @ wx
        Rt = torch.cat([R, torch.mul((V @ u[..., None]),s.unsqueeze(2))], dim=-1)
...
def world2cam(X, pose):  # [B,N,3]
    ...
    return (pose_inv @ X_hom.transpose(-1, -2)).transpose(-1, -2)
...
def cam2world(X, pose):
    X_hom = to_hom(X)
    return (pose @ X_hom.transpose(-1, -2)).transpose(-1, -2)
...
def get_center_and_ray_intr(opt, pose, poses,ray_idx, intr=None):  # [HW,2]
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    assert (opt.camera.model == "perspective")
    with torch.no_grad():
        # compute image coordinate grid
    ...
    # compute center and ray
    batch_size = len(pose)
    xy_grid = xy_grid.repeat(batch_size, 1, 1)  # [B,H*W,2]
    grid_3D = img2cam(to_hom(xy_grid), intr)[:,ray_idx]  # [B,H*W,3]
    center_3D = torch.zeros_like(grid_3D)  # [B,H*W,3]
    # 将每个张量复制 m 次
    poses_tensor = torch.stack(poses, dim=1)
    poses_tensor = poses_tensor.repeat(1, opt.W, 1, 1).view(batch_size, opt.W * opt.H, 3, 4)

    # 一帧上面只有一个pose，扩展第二个维度之后，并且将pose扩展成H * W多个
    # poses_tensor = pose
    # poses_tensor = poses_tensor.unsqueeze(1).expand(-1,opt.H*opt.W, -1, -1)

    poses_tensor = poses_tensor[:,ray_idx]
    grid_3D_result = cam2world(grid_3D[...,None,:],poses_tensor)[...,0,:]
    center_3D_result = cam2world(center_3D[...,None,:],poses_tensor)[...,0,:]
    ray_result = grid_3D_result - center_3D_result


    return center_3D_result, ray_result
