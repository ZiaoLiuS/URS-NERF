...
class RayDataset(Dataset):
    ...
    def compute_match_points(self):
        ...
        for i in range(len(cam_frames) - 1):
            # 获取相邻的两帧图像
            image0 = load_image(cam_frames[i]['image_filename']).cuda()
            image1 = load_image(cam_frames[i + 1]['image_filename']).cuda()
            ...
            twist_matches[(i, i + 1)] = (points0, points1)

        return twist_matches
